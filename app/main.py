...
def get_password_hash(password):
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Password hashing service temporarily unavailable"
        )
...
def seed_default_data():
    """Create the default admin account and a sample published job so the public AOCA workflow works out of the box."""
    try:
        admin_email = "admin@aoca.com"
        ...
        if not admin:
            ...
            admin_insert = db.users.insert_one(admin_user)
            admin_id = admin_insert.inserted_id
        else:
            admin_id = admin["_id"]

        if db.job_listings.count_documents({}) == 0:
            db.job_listings.insert_one(
                {
                    "title": "Frontend Developer",
                    "company": "AOCA Resources",
                    "description": "Build high-quality, responsive web interfaces for the AOCA learning platform.",
                    "requirements": [
                        "Bachelor's degree in Computer Science or a related field",
                        "At least 2 years of frontend experience",
                        "Strong knowledge of React and JavaScript",
                    ],
                    "responsibilities": [
                        "Build and maintain responsive frontend features",
                        "Collaborate with designers and developers",
                        "Ensure accessibility and maintainability of UI code",
                    ],
                    "location": {
                        "city": "Lagos",
                        "state": "Lagos State",
                    ...
                    "benefits": ["Remote work", "Health support", "Learning budget"],
                    "application_url": None,
                    "application_email": "careers@aoca.com",
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    user_payload = user.model_dump(exclude={"hashed_password"}, by_alias=True)
    return {
        "access_token": access_token,
        "token_type": "bearer",
        "user": parse_json(user_payload),
    }
...
async def create_blog_post(
    post: BlogPostCreate, current_user: User = Depends(get_current_active_user)
):
    """
...
async def delete_blog_post(
    post_id: str, current_user: User = Depends(get_current_active_user)
):
    """
...
async def apply_for_job(job_id: str, application: JobApplicationCreate):
    ...
    try:
        ...
        # Legacy AOCA records may still carry expired deadlines; public applications
        # should proceed unless the admin explicitly disables the listing.
...
async def get_user_dashboard(current_user: User = Depends(get_current_active_user)):
        ...
        courses.append(
            {
                **parse_json(c),
                ...
                "last_lesson_id": (
                    str(prog["last_lesson_id"])
                    if prog and prog.get("last_lesson_id")
                    else None
                ),
            }
        )
    ...
    upcoming_classes = list(
        db.classes.find(
            {"course_id": {"$in": course_ids}, "date": {"$gte": datetime.utcnow()}}
        )
        .sort("date", 1)
        .limit(5)
    )
    ...
    resources = (
        list(db.resources.find({"level": {"$in": levels}}).limit(6)) if levels else []
    )
...
async def get_dashboard_overview(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        # Get user's courses
        user_courses = list(
            db.user_courses.find({"user_id": ObjectId(current_user.id)})
        )
        ...
        for course in courses:
            progress = db.progress.find_one(
                {"user_id": ObjectId(current_user.id), "course_id": course["_id"]}
            )
...
async def get_user_assignments_dashboard(
    ...
    for a in assignments:
        ...
        submitted = any(
            str(s.get("user_id")) == str(user_oid) for s in a.get("submissions", [])
        )
        if submitted:
            a["status"] = "completed"
...
async def get_course_progress_fixed(
    ...
    completed_ids = [
        str(lid)
        for lid in (progress.get("completed_lesson_ids", []) if progress else [])
    ]
    ...
    all_lessons = []
    for mod in modules_from_collection:
    ...
    return {
        ...
        "percentage": pct,
        "last_lesson_id": (
            str(progress["last_lesson_id"])
            if progress and progress.get("last_lesson_id")
            else None
        ),
        "last_active": (
            progress["last_active"].isoformat()
            if progress and progress.get("last_active")
            else None
        ),
    }
...
@app.post(
    "/dashboard/progress/lesson/{lesson_id}/complete", response_model=Dict[str, Any]
)
...
async def get_my_certificates(
    ...
    certs = list(
        db.certificates.find({"user_id": ObjectId(str(current_user.id))}).sort(
            "issued_at", -1
        )
    )
...
async def verify_certificate(verification_code: str):
    ...
    if not cert:
        raise HTTPException(
            status_code=404, detail="Certificate not found or invalid code"
        )
...
async def create_course_module(
    course_id: str, module: ModuleCreate, admin_user: User = Depends(get_admin_user)
):
    """Admin creates a module in a course."""
    if not ObjectId.is_valid(course_id):
        raise HTTPException(status_code=400, detail="Invalid course ID")
    ...
    module_dict["course_id"] = ObjectId(course_id)
    module_dict["created_at"] = datetime.utcnow()

    result = db.modules.insert_one(module_dict)
    module_dict["_id"] = result.inserted_id

    return {"message": "Module created successfully", "module": parse_json(module_dict)}
...
async def create_module_lesson(
    module_id: str,
    lesson: LessonCreate,
    instructor_user: User = Depends(get_current_active_user),
):
    """Instructor creates a lesson in an assigned module."""
    if instructor_user.get("role") not in ["admin", "instructor"]:
    ...
    lesson_dict["module_id"] = ObjectId(module_id)
    lesson_dict["created_at"] = datetime.utcnow()

    result = db.lessons.insert_one(lesson_dict)
    lesson_dict["_id"] = result.inserted_id

    return {"message": "Lesson created successfully", "lesson": parse_json(lesson_dict)}


@app.post(
    "/instructor/courses/{course_id}/assessments",
    response_model=Dict[str, Any],
    status_code=status.HTTP_201_CREATED,
)
async def create_assessment(
    ...
    """Instructor creates an assessment (quiz/test/assignment) for a course."""
    if instructor_user.get("role") not in ["admin", "instructor"]:
        raise HTTPException(status_code=403, detail="Not authorized")
    ...
    assessment_dict["course_id"] = ObjectId(course_id)
    assessment_dict["created_at"] = datetime.utcnow()

    result = db.assessments.insert_one(assessment_dict)
    assessment_dict["_id"] = result.inserted_id

    return {
        "message": "Assessment created successfully",
        "assessment": parse_json(assessment_dict),
    }


@app.post(
    "/students/courses/{course_id}/enroll-request",
    response_model=Dict[str, Any],
    status_code=status.HTTP_201_CREATED,
)
async def request_course_enrollment(
    course_id: str, current_user: User = Depends(get_current_active_user)
):
    ...
    # FIX: use .id attribute consistently (same as all other endpoints)
    user_oid = ObjectId(current_user.id)

    # Check existing enrollment
    existing = db.user_courses.find_one(
        {"user_id": user_oid, "course_id": ObjectId(course_id)}
    )
    if existing:
        return {
    ...
    result = db.user_courses.insert_one(enrollment)
    return {
...
async def get_my_enrollments(current_user: User = Depends(get_current_active_user)):
    """
    ...
    for rec in records:
        ...
        class_ = (
            db.classes.find_one({"_id": rec.get("class_id")})
            if rec.get("class_id")
            else None
        )
        result.append(
            {
                **parse_json(rec),
                ...
                "class_date": (
                    class_.get("date").isoformat()
                    if class_ and class_.get("date")
                    else None
                ),
...
async def get_instructor_stats_v2(
    ...
    try:
        ...
        total_courses = db.courses.count_documents(instructor_match)
        instructor_courses = list(db.courses.find(instructor_match, {"_id": 1}))
...
@app.get("/instructor/assessments", response_model=List[Dict[str, Any]])
async def get_instructor_assessments(
    current_user: User = Depends(get_instructor_user),
    course_id: Optional[str] = Query(None),
):
    """Get all assessments for the current instructor's courses."""
    try:
        user_oid = ObjectId(str(current_user.id))
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid user ID")
    ...
    assessments = list(db.assessments.find(query).sort("due_date", 1))

    for a in assessments:
        a.setdefault("title", a.get("title", ""))
        a.setdefault("type", a.get("type", "quiz"))
        a.setdefault("questions", a.get("questions", []))
        a.setdefault("passing_score", a.get("passing_score"))
        a.setdefault("due_date", a.get("due_date"))
        course = db.courses.find_one({"_id": a.get("course_id")})
        if course:
            a["course"] = {
                "id": str(course["_id"]),
    ...
    return parse_json(assessments)


@app.delete(
    "/instructor/assessments/{assessment_id}", status_code=status.HTTP_200_OK
)
async def delete_instructor_assessment(
    assessment_id: str,
    current_user: User = Depends(get_instructor_user),
):
    """Delete an assessment by the instructor who created it or by admin."""
    if not ObjectId.is_valid(assessment_id):
        raise HTTPException(status_code=400, detail="Invalid assessment ID")

    existing = db.assessments.find_one({"_id": ObjectId(assessment_id)})
    if not existing:
        raise HTTPException(status_code=404, detail="Assessment not found")

    db.assessments.delete_one({"_id": ObjectId(assessment_id)})
    return {"success": True, "message": "Assessment deleted successfully"}


@app.get(
    "/instructor/courses/{course_id}/modules", response_model=Dict[str, Any]
)
async def get_instructor_course_modules(
    course_id: str, current_user: User = Depends(get_instructor_user)
):
    """Get modules for an instructor's assigned course."""
    if not ObjectId.is_valid(course_id):
        raise HTTPException(status_code=400, detail="Invalid course ID")

    instructor_match = {
        "$or": [
            {"instructor_id": ObjectId(str(current_user.id))},
            {"instructor_id": str(current_user.id)},
            {"instructor_id": current_user.id},
        ]
    }
    if not db.courses.find_one({"_id": ObjectId(course_id), **instructor_match}):
        raise HTTPException(
            status_code=403, detail="Not assigned to this course"
        )

    modules = list(
        db.modules.find({"course_id": ObjectId(course_id)}).sort("order", 1)
    )
    return {"modules": parse_json(modules)}


# ── Instructor create class/session ────────────────────────────
@app.post("/instructor/classes", response_model=Dict[str, Any])
async def instructor_create_class(
    body: dict,
    current_user: User = Depends(get_instructor_user),
):
    """Instructor creates a new class/session for their assigned course."""
    ...
    course = db.courses.find_one({"_id": ObjectId(body["course_id"])})
    if not course:
        raise HTTPException(status_code=404, detail="Course not found")
    ...
    if not db.courses.find_one({"_id": course["_id"], **instructor_match}):
        raise HTTPException(
            status_code=403, detail="Not assigned to this course"
        )
...
@app.get("/instructor/resources", response_model=Dict[str, Any])
async def get_instructor_resources(
    current_user: User = Depends(get_instructor_user),
):
    """Get all resources for courses assigned to the current instructor."""
    instructor_match = {
        "$or": [
            {"instructor_id": ObjectId(str(current_user.id))},
            {"instructor_id": str(current_user.id)},
    ...
    resources = list(db.resources.find({"course_id": {"$in": course_ids}}))
    return {"resources": parse_json(resources)}


@app.post("/instructor/resources", response_model=Dict[str, Any])
async def instructor_create_resource(
    body: dict,
    current_user: User = Depends(get_instructor_user),
):
    """Instructor creates a resource for their assigned course."""
    required = ["title", "description", "type", "url", "level", "category"]
    for field in required:
        if not body.get(field):
            raise HTTPException(status_code=400, detail=f"{field} is required")
    ...
    created = db.resources.find_one({"_id": result.inserted_id})
    return parse_json(created)


@app.get("/instructor/courses/list", response_model=Dict[str, Any])
async def get_instructor_courses_list(
    current_user: User = Depends(get_instructor_user),
):
    ...
    instructor_match = {
        "$or": [
            {"instructor_id": user_oid},
            {"instructor_id": user_id_str},
            {"instructor_id": current_user.id},
        ]
    }

    raw_courses = list(db.courses.find(instructor_match))
    print(
    ...
    for raw in raw_courses[:3]:
        print(
            " instructor course candidate",
            "course_id=",
            raw.get("_id"),
            "instructor_id=",
            raw.get("instructor_id"),
            "instructor_id_type=",
            type(raw.get("instructor_id")).__name__,
        )
    ...
    return {"courses": parse_json(courses)}
...
async def update_user(
    user_id: str, user_update: UserUpdate, admin_user: User = Depends(get_admin_user)
):
    """
...
async def approve_enrollment(
    enrollment_id: str, body: dict = {}, admin_user: User = Depends(get_admin_user)
):
    """
    ...
    if rec["status"] not in ("pending",):
        raise HTTPException(
            status_code=400, detail=f"Cannot approve — current status: {rec['status']}"
        )
    ...
    return {
        "message": "Student approved and enrolled. They now have course access.",
...
async def assign_student(body: dict, admin_user: User = Depends(get_admin_user)):
    ...
    for field, val in [
        ...
        if not val or not ObjectId.is_valid(val):
            raise HTTPException(
                status_code=400, detail=f"{field} is required and must be valid"
            )
...
async def reassign_student(body: dict, admin_user: User = Depends(get_admin_user)):
    ...
    if rec.get("status") != "active":
        raise HTTPException(
            status_code=400, detail="Student is not currently active in this course"
        )
...
async def remove_student_from_class(
    body: dict, admin_user: User = Depends(get_admin_user)
):
    """
...
async def get_class_students(class_id: str, admin_user: User = Depends(get_admin_user)):
    ...
    enrollments = list(
        db.user_courses.find({"class_id": class_oid, "status": "active"})
    )
    ...
    for s in students:
        ...
        progress = db.progress.find_one(
            {"user_id": s["_id"], "course_id": class_doc.get("course_id")}
        )
        result.append(
            {
...
async def get_course_by_id(course_id: str, admin_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        for student in students:
            enrollment = next(
                (e for e in enrollments if e["user_id"] == student["_id"]), None
            )
            formatted_students.append(
                {
                    ...
                    "enrollment_date": (
                        enrollment.get("enrolled_at") if enrollment else None
                    ),
                }
            )
        ...
        # Get classes
        classes = list(
            db.classes.find({"course_id": ObjectId(course_id)}).sort("date", 1)
        )
        course["classes"] = parse_json(classes)

        # Get assignments
        assignments = list(
            db.assignments.find({"course_id": ObjectId(course_id)}).sort("due_date", 1)
        )
...
async def admin_create_course(
    ...
    # Convert duration to correct types
    if "duration" in body:
...
async def enroll_user_in_course(
    course_id: str, user_id: str, admin_user: User = Depends(get_admin_user)
):
    """
...
@app.delete(
    "/admin/courses/{course_id}/enroll/{user_id}", status_code=status.HTTP_200_OK
)
...
async def get_course_students(
    ...
    enrollments = list(
        db.user_courses.find({"course_id": ObjectId(course_id)}).skip(skip).limit(limit)
    )
    ...
    for enc in enrollments:
        ...
        class_doc = (
            db.classes.find_one({"_id": enc.get("class_id")})
            if enc.get("class_id")
            else None
        )
        progress = db.progress.find_one(
            {"user_id": enc["user_id"], "course_id": ObjectId(course_id)}
        )
        ...
        result.append(
            {
                ...
                "enrolled_at": (
                    enc.get("enrolled_at").isoformat()
                    if enc.get("enrolled_at")
                    else None
                ),
...
async def get_admission_inquiries(
    ...
    try:
        ...
        inquiries = list(
            db.admission_inquiries.find(query)
            .sort(sort_by, sort_order)
            .skip(skip)
            .limit(limit)
        )
        ...
        # Get statistics
        status_stats = list(
            db.admission_inquiries.aggregate(
...
async def get_admission_inquiry(
    inquiry_id: str, admin_user: User = Depends(get_admin_user)
):
    """
    ...
    try:
        ...
        db.admission_inquiries.update_one(
            {"_id": ObjectId(inquiry_id)}, {"$set": {"is_read": True}}
        )
...
@app.put(
    "/admin/admission-inquiries/{inquiry_id}/status", status_code=status.HTTP_200_OK
)
async def update_inquiry_status(
    inquiry_id: str, status_data: dict, admin_user: User = Depends(get_admin_user)
):
    """
...
@app.post(
    "/admin/admission-inquiries/{inquiry_id}/notes", status_code=status.HTTP_200_OK
)
async def add_inquiry_note(
    inquiry_id: str, note_data: dict, admin_user: User = Depends(get_admin_user)
):
    """
...
async def admin_get_careers_stats(admin_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        recent_applications = list(
            db.job_applications.find().sort("created_at", -1).limit(5)
        )
        ...
        top_applied_jobs = list(
            db.job_listings.find().sort("applications_count", -1).limit(5)
        )
...
async def admin_get_job_listings(
    ...
    try:
        ...
        jobs = list(
            db.job_listings.find(query).sort("created_at", -1).skip(skip).limit(limit)
        )

        total = db.job_listings.count_documents(query)

        for job in jobs:
            job["applications_count"] = db.job_applications.count_documents(
                {"job_id": job["_id"]}
            )
...
async def admin_create_job_listing(
    job: JobListingCreate, admin_user: User = Depends(get_admin_user)
):
    """
...
async def admin_delete_job_listing(
    job_id: str, admin_user: User = Depends(get_admin_user)
):
    """
...
@app.get(
    "/admin/careers/applications/{application_id}",
    response_model=JobApplicationResponse,
)
async def admin_get_application_details(
    application_id: str, admin_user: User = Depends(get_admin_user)
):
    """
...
async def admin_create_job_category(
    category: JobCategoryCreate, admin_user: User = Depends(get_admin_user)
):
    """
...
async def admin_update_job_category(
    ...
    try:
        ...
        updated["job_count"] = db.job_listings.count_documents(
...
@app.delete(
    "/admin/careers/categories/{category_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def admin_delete_job_category(
    category_id: str, admin_user: User = Depends(get_admin_user)
):
    """
...
async def get_admin_dashboard_stats(admin_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        courses_by_level = list(
            db.courses.aggregate([{"$group": {"_id": "$level", "count": {"$sum": 1}}}])
        )
        ...
        recent_inquiries = list(
            db.admission_inquiries.find().sort("created_at", -1).limit(5)
        )
...
async def get_contact_submission_details(
    submission_id: str, admin_user: User = Depends(get_admin_user)
):
    """
    ...
    try:
        ...
        db.contact_submissions.update_one(
            {"_id": ObjectId(submission_id)}, {"$set": {"is_read": True}}
        )
...
@app.put(
    "/admin/contact-submissions/{submission_id}/read", status_code=status.HTTP_200_OK
)
...
@app.delete(
    "/admin/contact-submissions/{submission_id}", status_code=status.HTTP_200_OK
)
